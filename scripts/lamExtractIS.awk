		...
		if(pos=match($0,"AS:i:")) {
			...
			if (AS[3]>0) {
				...
				if (XS[3]/AS[3] >= alScore) {
					print $0 > fileName[1]"_"ml".is.repeats.OKtoMerge"
	...
	if ((match($3,"chr"))&&($2<=16)&&(length($10)>=ml)) {
		...
		if ((length(cigar1)==1)&&(length(cigar2)==3)&&((match($6,"I"))||(match($6,"D")))) {
			...
			if ( numMatch>=ml)
				...
				print $1" "$3" "$4+delta" "$2" "$6" " numMatch " "$10 > fileName[1]"_"ml".is"
				#print $1" "$3" "$4+delta" "$2" "$6" " numMatch " "$10 
			else 
				 print $0 "-> Seq length smaller than " ml   > fileName[1]"_"ml".is.removed"
			next;
		}
		...
		if ((length(cigar1)==2)&&(length(cigar2)==3)&&((match($6,"I"))||(match($6,"D")))) {
			...
			if ( numMatch>=ml)
				print $1" "$3" "$4+delta" "$2" "$6" " numMatch " "sequenza > fileName[1]"_"ml".is"
		...
		if(length(cigar1)==3 && length(cigar2)==2) {
			...
			#print IS1[1]
			if(IS1[1]>=ml)
				print $1" "$3" "$4+delta" "$2" "$6" "IS1[1]" "$10 > fileName[1]"_"ml".is"
			else 
				print $0 "-> Seq length smaller than " ml   > fileName[1]"_"ml".is.removed"
			next
		}
		...
		if (length(cigar1)==1) {
				...
				if(cigar2[1]>=ml)
					print $1" "$3" "$4+delta" "$2" "$6" "cigar2[1]" "$10 > fileName[1]"_"ml".is"
		...
		if ((length(cigar1)<=2)&&(length(cigar2)<=2)) {
				...
				if (length(IS1)==2) {
					...
					if(IS1[1]>=ml)
						print $1" "$3" "$4+delta" "$2" "$6" "IS1[1]" "sequenza> fileName[1]"_"ml".is"
					...
					if(IS2[1]>=ml)
						print $1" "$3" "$4+delta" "$2" "$6" "IS2[1]" "sequenza> fileName[1]"_"ml".is"
		...
		print $0 "-> No rules for this sequence"   > fileName[1]"_"ml".is.removed"
	}
}
