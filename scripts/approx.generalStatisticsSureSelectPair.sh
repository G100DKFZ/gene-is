...
vectorString=$3
samtools=$4
#other files from output/working dir
...
echo " " >> $output_dir/GeneralStatistics.txt
echo "Number of APPROXIMATE unclustered Integration Sites (sequence_count) " >> $output_dir/GeneralStatistics.txt
cat  $output_dir/Approx.ResultsCompleteUnClustered.csv | tail -n +2 | wc -l >> $output_dir/GeneralStatistics.txt
echo " " >> $output_dir/GeneralStatistics.txt
echo "Number of APPROXIMATE clustered Integration Sites" >> $output_dir/GeneralStatistics.txt
cat  $output_dir/Approx.ResultsClusteredAnnotated.csv |  tail -n +2 | wc -l >> $output_dir/GeneralStatistics.txt
echo " " >> $output_dir/GeneralStatistics.txt
